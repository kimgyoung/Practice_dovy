...
public class UserController {
    ...
    @PostMapping("/join")
    public ResponseEntity<?> join (@RequestBody @Valid UserRequest.LoginDTO requestDTO, Error error) {
        userService.join(requestDTO);
        return ResponseEntity.ok(ApiUtils.success(null) );
    }

    @PostMapping("/check")
    public ResponseEntity<?> check (@RequestBody @Valid UserRequest.LoginDTO requestDTO, Error error){
